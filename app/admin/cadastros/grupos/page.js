...
export default function Page() {
  ...
  function editar(item) {
    setEditId(item.id);
    setForm({ nome: item.nome || "", ordem: item.ordem ?? 0, ativo: !!item.ativo });
  }
  ...
  async function salvar(e) {
    ...
    const payload = { nome: form.nome?.trim(), ordem: Number(form.ordem || 0), ativo: !!form.ativo };
    ...
    if (editId) {
      const { error } = await supabase.from("grupos").update(payload).eq("id", editId);
  ...
  return (
    <RequireRole allow={["admin"]}>
      <div className="space-y-4">
        <PageTitle
          title="Grupos"
          subtitle="Cadastro de grupos da organização."
          right={
